<script lang="ts">
	import { resolve } from "$app/paths";
	import { auth } from "$lib/auth/auth.svelte";
	const steps = [
		{
			kicker: "01",
			title: "What is the Goal",
			text: "Describe what a successful interaction with your dog would look like."
		},
		{
			kicker: "02",
			title: "Where Are We Now",
			text: "Identify relevant skills, reinforcers, and conditions that can be used to begin."
		},
		{
			kicker: "03",
			title: "How Do We Get There",
			text: "Determine where the target pattern already occurs and under what conditions."
		},
		{
			kicker: "04",
			title: "Build Your Starting Strategy",
			text: "Receive a starting point based on the Constructional Affection methodology."
		}
	];
</script>

<svelte:head>
	<title>Constructional Affection Coach</title>
	<meta
		name="description"
		content="A guided interview that helps determine where to begin a Constructional Affection program."
	/>
</svelte:head>

<main class="min-h-screen bg-background text-foreground">
	<section class="relative overflow-hidden bg-primary text-primary-foreground">
		<div
			class="relative mx-auto grid max-w-7xl items-center gap-10 px-6 pt-10 pb-24 sm:pb-0 lg:grid-cols-[1fr_0.95fr] lg:px-10 lg:pb-32"
		>
			<div class="relative max-w-2xl">
				<p class="eyebrow mb-5">Guided Constructional Questionnaire</p>

				<h1 class="text-5xl leading-[1.05] font-bold text-white sm:text-6xl lg:text-7xl">
					Build the interaction you want with your dog.
				</h1>

				<p class="mt-6 max-w-xl text-lg leading-8 text-white/85">
					A guided interview that helps identify the outcome you want, the skills and reinforcers
					already available, and where to begin a Constructional Affection program.
				</p>

				<div class="z-10 mt-8 flex flex-col gap-3 sm:flex-row">
					<a
						href={resolve("/interview")}
						class="button-base button-primary z-10 inline-flex items-center justify-center px-6 py-4"
					>
						Start the Interview <span class="ml-3">→</span>
					</a>

					{#if auth.isAuthenticated}
						<a
							href={resolve("/programs")}
							class="button-base button-secondary z-10 inline-flex items-center justify-center px-6 py-4"
						>
							Saved Programs
						</a>{/if}
				</div>
			</div>

			<div class="relative h-80 sm:h-96 lg:min-h-190 lg:overflow-visible">
				<img
					src="/images/nanook_hero.png"
					alt="Calm dog lying down"
					class="right-0 bottom-21 -mt-12 w-110 max-w-none drop-shadow-2xl sm:absolute sm:bottom-50 sm:mt-0 sm:w-136 lg:-right-16 lg:bottom-60 lg:w-184"
				/>
			</div>
		</div>

		<div
			class="absolute right-0 bottom-0 left-0 h-24 bg-linear-to-t from-background to-transparent"
		></div>
	</section>

	<section class="relative z-20 mx-auto -mt-40 max-w-6xl px-6 sm:-mt-40 lg:mt-0 lg:px-10">
		<div class="rounded-vintage border border-border bg-white p-6 shadow-soft sm:p-8">
			<div class="flex flex-col gap-5 md:flex-row md:items-center">
				<div
					class="flex h-20 w-20 shrink-0 items-center justify-center rounded-full bg-primary text-3xl text-accent"
				>
					◎
				</div>

				<div>
					<h2 class="font-body text-xl font-bold text-primary">
						Constructional Affection is designed to help owners construct calm, patient interaction
						patterns using affection as reinforcement.
					</h2>
					<p class="mt-2 text-muted-dark">
						It is not a general-purpose dog training or behavior-treatment program.
					</p>
				</div>
			</div>
		</div>
	</section>

	<section class="mx-auto max-w-7xl px-6 py-20 lg:px-10">
		<div class="text-center">
			<h2 class="text-4xl font-bold text-primary">How It Works</h2>
			<div class="mx-auto mt-4 h-1 w-20 rounded-full bg-accent"></div>
		</div>

		<div class="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
			{#each steps as step (step.title)}
				<article class="rounded-vintage border border-border bg-white p-6 text-center shadow-soft">
					<div
						class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-primary text-lg font-bold text-accent"
					>
						{step.kicker}
					</div>

					<h3 class="mt-5 font-body text-xl font-bold text-primary">{step.title}</h3>
					<p class="mt-4 text-sm leading-7 text-muted-dark">{step.text}</p>
				</article>
			{/each}
		</div>
	</section>

	<section class="mx-auto max-w-7xl px-6 pb-16 lg:px-10">
		<div class="rounded-vintage bg-primary p-10 text-center shadow-soft">
			<h2 class="mx-auto max-w-2xl text-4xl font-bold text-white">
				Ready to build a better interaction with your dog?
			</h2>

			<a href={resolve("/interview")} class="button-base button-primary mt-8 inline-flex px-8 py-4">
				Start the Interview <span class="z-10 ml-3">→</span>
			</a>
		</div>
	</section>
</main>
